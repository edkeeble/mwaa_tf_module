...
data "aws_iam_policy_document" "mwaa" {
  statement {
    ...
    resources = [
      "arn:${data.aws_partition.current.id}:airflow:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:environment/${var.mwaa_env_name}"
    ]
  }
  statement {
    effect = "Allow"
  ...
  statement {
    ...
    resources = [
      "arn:${data.aws_partition.current.id}:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:airflow-${var.mwaa_env_name}-*"
    ]
  }
...
data "aws_iam_policy_document" "lambda_s3_event_policy" {
  statement {
    effect = "Allow"
    actions = [
